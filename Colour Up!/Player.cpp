...
void Player::getNewSpeed(double dt){
	...
	if (applyGravity){
		...
		if (moveRequestUp && !jumping && (currentSpeedY > -40 && currentSpeedY < 40)){
			//jump
			jumping = true;
...
void Player::modifySpeed(float pushDistRight, float pushDistLeft, float pushDistDown, float pushDistUp, double dt){
	...
	//PLAYER HAS TO BE PUSHED UP (ADD TO VELOCITY)
...
void Player::initialise(){
	...
	jumpStartSpeedYOriginal = jumpStartSpeedY;
...
void Player::changeTexture(Action action){
	switch(action){
	case IDLE:
		if (jumpStartSpeedY != jumpStartSpeedYOriginal){
